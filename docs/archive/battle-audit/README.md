...
> Archived baseline: all 26 findings and the subsequently confirmed calculation/authorization repairs are complete. See [repair verification](verification-results.md), the [maintained regression coverage](../../../tests/battle/README.md), and the [current release checklist](../../release-checklist.md). Source locations and observations below describe the audited snapshot.
...
These original standalone checks were superseded by the maintained isolated-process suite under `tests/battle/`; run it with `bun run test:release` from the repository root. The old scripts, logs and seeded encounter traces remain available in Git commit `396d798` under `plans/battle-audit/`.

The original audit exercised combat and client code, with doubles at platform boundaries. It did not exercise the production database, Cloudflare scheduler, authenticated browser session or visual rendering.
