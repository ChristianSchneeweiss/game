...
The [approved specification](spec.md) settles the original design destination, and the [spell conversion table](spell-conversion.md) records the accepted defaults. Remaining work follows the [release checklist](../../production/checklist.md), including real authenticated multi-account and deployed networking checks. A full tier/scaling redesign remains outside this conversion's scope.
