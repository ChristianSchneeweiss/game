...
Complete `release:check` passed using the isolated PostgreSQL proof environment. Reviewed [PostgreSQL proof](evidence/postgresql-proof.json), [artifact report](evidence/production-artifact.json), [asset validation](evidence/assets.json) and [browser measurements](evidence/browser-performance.json) are retained beside this document. Raw evidence: `/tmp/loot-production-candidate-final/`; console: `/tmp/loot-production-candidate-final-console.log`.
...
See [browser qualification](browser.md) for observations, measurements and the remaining checklist. Screenshots are visible in the task, not committed portable fixtures.
...
The [release checklist](checklist.md) is the active record for candidate diagnostics, dependency dispositions, isolated staging/runtime proof, complete authenticated browser acceptance and promotion. This document records completed observations; the local candidate is not yet release-qualified.
...
[Setup/commands](setup.md) · [Architecture](architecture.md) · [Client lifetimes](client.md) · [Security/permissions](security-review.md) · [Migration/release/recovery](release.md)
