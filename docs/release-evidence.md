...
- Code candidate: **`4ae05e3a56b551137590860e87d7a20c6a7b6d18`**. The complete source tested below is committed there; subsequent documentation commits change no application, release-tool, configuration or maintained test source.
...
Complete `release:check` passed using the isolated PostgreSQL proof environment. Reviewed [PostgreSQL proof](release-evidence/postgresql-proof.json), [artifact report](release-evidence/production-artifact.json), [asset validation](release-evidence/assets.json) and [browser measurements](release-evidence/browser-performance.json) are retained beside this document. Raw evidence: `/tmp/loot-production-candidate-final/`; console: `/tmp/loot-production-candidate-final-console.log`.
...
See [browser qualification](production-browser.md) for observations, measurements and the remaining checklist. Screenshots are visible in the task, not committed portable fixtures.
...
The [release checklist](release-checklist.md) is the active record for candidate diagnostics, dependency dispositions, isolated staging/runtime proof, complete authenticated browser acceptance and promotion. This document records completed observations; the local candidate is not yet release-qualified.
...
[Setup/commands](production-setup.md) · [Architecture](production-architecture.md) · [Client lifetimes](production-client.md) · [Security/permissions](production-security-review.md) · [Migration/release/recovery](production-release.md)
