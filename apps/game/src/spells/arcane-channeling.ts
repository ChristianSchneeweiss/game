...
export class ArcaneChannelingSpell extends ApplyStatusSpell {
  ...
  constructor(id: string) {
    super(
      ...
      new EffectModule(
        ({ caster, roll, target }) =>
          new ChargeEffect(2, () => this.chargeAction({ caster, roll, target }))
      )
    );
  }
  ...
  private chargeAction(args: {
    ...
    if (!this.validateTargets(args.caster, [args.target])) {
      return;
    }
