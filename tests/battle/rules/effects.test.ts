...
// Test the shared damage, attribute and effect lifecycle interfaces. Named
// content supplies real rule combinations; no resolver methods are replaced.
function character(
  id: string,
  team: "TEAM_A" | "TEAM_B",
...
describe("damage reactions and effect lifecycle", () => {
  for (const duration of [1, 2, 3]) for (const agility of [19, 21]) for (const kind of ["damage", "healing"] as const) {
    test(`${kind} over ${duration} rounds ticks ${duration} times with caster agility ${agility}`, () => {
      const caster = character("caster", "TEAM_A");
      const target = character("target", "TEAM_B");
      caster.baseAttributes.agility = agility;
      target.health = 100;
      const bm = battle(caster, target);
      if (bm.getCurrentRound().orderQueue[0] === target.id) bm.postTurn();
      const effect = kind === "damage" ? new DamageOverTimeEffect(duration, 5, "MAGICAL") : new HealingOverTimeEffect(duration, 5);
      bm.handler.effect(caster.spells[0]!, effect, caster, target);
      for (let turn = 0; turn < (duration + 2) * 2; turn++) bm.postTurn();
      expect(target.health).toBe(kind === "damage" ? 100 - duration * 5 : 100 + duration * 5);
      expect(target.activeEffects).not.toContain(effect);
      const expiredHealth = target.health;
      for (let turn = 0; turn < 4; turn++) bm.postTurn();
      expect(target.health, "expired effects stop ticking").toBe(expiredHealth);
    });
  }
  test("Bloodfang permits magical spells and publishes their completed cast", () => {
  ...
  test("Soulleech permits Basic Attack", () => {
  ...
  test("control: matching physical lifesteal completes and heals", () => {
  ...
  test("Aegis Wall completes and attaches a shield", () => {
  ...
  test("opposing Thorn Carapaces finish a hit without recursive reflection", () => {
  ...
  test("Deflecting Stance and opposing Thorn Carapace finish a hit", () => {
  ...
  test("critical damage lookup includes the configured base critical multiplier", () => {
    const caster = character("caster", "TEAM_A");
    const target = character("target", "TEAM_B");
    const bm = battle(caster, target);
  ...
  test("Mystic Flow and Vital Wellspring improve actual upkeep regeneration", () => {
  ...
  test("Titan's Resurgence registers a healing effect that ticks and expires safely", () => {
    ...
    expect(bm.effectTracking.has(hot.id), "passive effects are visible in replay").toBe(true);
    bm.onPostRound();
  ...
  test("lifesteal cannot revive a caster killed by the target's reflection", () => {
    const caster = character("caster", "TEAM_A", ["basic-attack"], ["bloodfang"]);
    const target = character("target", "TEAM_B", ["basic-attack"], ["thorn-carapace"]);
    const bm = battle(caster, target);
    caster.health = 1;
  ...
  test("a two-turn Cinderbrand burn ticks twice regardless of caster initiative", () => {
  ...
  test("control: stacked stat modifiers expire independently and restore the base stat", () => {
    const caster = character("caster", "TEAM_A");
    const target = character("target", "TEAM_B");
    const bm = battle(caster, target);
