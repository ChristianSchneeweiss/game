...
test("real client hook displays server mana after delayed charge and reconnect", async () => {
  ...
  const caster = bm.getEntityById("hero-0")!;
  for (let step = 0; step < 12 && caster.activeEffects.some((e) => e.effectType === "CHARGE"); step++) {
    ...
    castBattleSpell(bm, { entityId: actor.id, spellId: `${actor.id}-basic-attack`, targetIds: [target.id] }, "fixture-owner");
  }
  expect(caster.activeEffects.some((e) => e.effectType === "CHARGE")).toBe(false);
  await act(async () => sendState());
  await act(async () => session.playback.skip());
  const liveMana = bm.getEntityById("hero-0")!.mana;
  const observedBeforeReconnect = session.playback.stats.get("hero-0")!.mana;
  // Rebuild the hook and socket, then receive the authoritative history again.
  await act(async () => root.unmount());
  root = createRoot(container);
  await act(async () => root.render(<Probe />));
  socket = sockets.at(-1)!;
...
test("a cast with an unsuccessful optional proc completes and keeps client resources consistent", async () => {
