...
// Audit regressions: these assert the required invariant and intentionally fail
// on the audited checkout. Application source is not changed by this audit.
...
test("charge release must not charge mana a second time in client reconstruction", () => {
  ...
  bm.start();
  cast(bm, caster, "arcane-channeling");
  const release = bm.events.filter(({ event }) =>
    event.eventType === "SPELL_CAST" && event.data.spellId === "caster-arcane-channeling"
  );
  expect(release).toHaveLength(2);
  const displayed = buildTimeline(bm.startEntityData, bm.events, undefined, bm.effectTracking).at(-1)!;
  expect(displayed.stats.get(caster.id)!.mana).toBe(caster.mana);
});
...
test("client preserves reflected damage followed by capped Soulflare healing", () => {
  ...
  const displayed = buildTimeline(bm.startEntityData, bm.events, undefined, bm.effectTracking).at(-1)!;
  expect(displayed.stats.get(caster.id)!.health).toBe(caster.health);
});
